...
5. [에디터 자동화 및 커스텀 컴포넌트 개발](#5-에디터-자동화-및-커스텀-컴포넌트-개발)
6. [오토플레이 시스템 개발](#6-오토플레이-시스템-개발)
7. [회전팡 최적 각도 자동 계산](#7-회전팡-최적-각도-자동-계산)
8. [셔플 시 특수 블록 조합 배치](#8-셔플-시-특수-블록-조합-배치)
9. [인게임 스태틱 시트 로딩 개편](#9-인게임-스태틱-시트-로딩-개편)
...
- 실 데이터 기반 시뮬레이션 (일반적인 라이브 상태의 월드맵) 기준으로 개선 성능 측정
...
## 6. 오토플레이 시스템 개발
레벨 담당자들의 반복 플레이 검증 업무와 QA 수동 테스트 공수를 줄이기 위해
**휴리스틱 기반의 오토플레이 시스템**을 개발했습니다.
이후 Unity ML-Agents를 활용한 강화학습 기반 확장을 시도하여 사내 세미나에서 발표했습니다.

---

### 💬 담당 역할 및 기여
- 매치3 퍼즐 게임의 자동 플레이를 위한 휴리스틱 기반 판단 로직 설계 및 구현
- QA 수동 테스트 공수 제거 및 레벨 담당자 반복 플레이 검증 업무 자동화
- Unity ML-Agents를 활용한 강화학습 기반 확장 연구 및 세미나 발표
- 라이브 환경에서의 랜덤 시드 고정 및 고속 시뮬레이션 가능 여부 등 지속적 RND 진행

#### 📈 개선 효과 요약
| 항목 | 개선 전 | 개선 후 |
|------|---------|---------|
| 레벨 검증 방식 | 담당자가 직접 반복 플레이 | **오토플레이 시스템으로 자동 검증** |
| QA 수동 테스트 | 매 업데이트마다 수동 테스트 필요 | **자동화로 공수 제거** |
| 시뮬레이션 | 불가능 | **랜덤 시드 고정 기반 재현 가능한 시뮬레이션 구축 중** |
...
##### 🎯 문제점
- 레벨 담당자가 맵 밸런스 검증을 위해 동일 스테이지를 수십 회 반복 플레이
- QA에서도 매 업데이트마다 수동으로 전체 스테이지 플레이 테스트 필요
- 검증에 투입되는 리소스가 다른 업무를 압박

##### 🧭 개선 방향
- 보드 상태를 분석하여 최적의 매칭을 판단하는 휴리스틱 로직 구현
...
##### 🧭 연구 방향
- Unity ML-Agents를 활용하여 강화학습 기반의 자동 플레이 에이전트 학습 시도
- 휴리스틱 대비 더 다양한 전략을 학습할 수 있는 가능성 확인
- 사내 세미나 '매치3 퍼즐 게임 자동플레이 - 밸런싱 자동화와 ML 도전' 주제로 발표

----

## 7. 회전팡 최적 각도 자동 계산
인게임에서 회전팡(직선 범위 특수 블록) 사용 시, 플레이어가 터치만으로 바로 발사할 수 있도록
**최적 방향을 자동으로 계산하는 기능**을 Burst 컴파일러를 활용하여 구현했습니다.

---

### 💬 담당 역할 및 기여
- 직사각형 범위 기반으로 닿는 오브젝트 수를 계산하는 수학적 로직 설계
- Burst 컴파일러를 활용한 고성능 연산 구현
- 옵션에서 사용자가 자동 사용 기능을 켜고 끌 수 있도록 설정 UI 연동

### 🛠 개선 내용 상세

#### 🎯 문제점
- 회전팡 사용 시 플레이어가 직접 각도를 조절해야 하는 불편함
- 최적의 방향을 찾기 위해 여러 번 조작이 필요

#### 🧭 개선 방향
- 모든 각도에 대해 직사각형 범위 내 타격 가능한 오브젝트를 계산
...
## 8. 셔플 시 특수 블록 조합 배치
셔플 발생 시 단순 랜덤 재배치가 아닌, **한 번의 스왑으로 회전팡(5개 일렬 매칭)을 만들 수 있는 패턴**이
배치되도록 하는 기능을 구현했습니다.

---

### 💬 담당 역할 및 기여
- 매치3에서 한 번 스왑으로 5개가 일렬로 연결되는 모든 프리-패턴 12가지를 도출
- 셔플 시 해당 패턴 중 하나가 보드에 배치 가능한지 검증하고, 가능하면 적용하는 로직 구현
- 패턴 배치 불가 시 일반 셔플로 폴백 처리

### 🛠 개선 내용 상세

#### 🎯 문제점
- 기존 셔플은 완전 랜덤으로 재배치되어, 셔플 후에도 좋은 수가 없는 경우가 빈번
- 유저 경험 측면에서 셔플 이후에도 답답한 상황이 지속
...
## 9. 인게임 스태틱 시트 로딩 개편
맵 데이터와 스태틱 시트 양쪽에서 중복으로 사용되는 데이터를 정리하여,
**운영 공수를 줄이고 불필요한 로딩을 제거**하는 개편 작업을 진행했습니다.

---

### 💬 담당 역할 및 기여
- 맵 데이터와 스태틱 시트에서 중복 사용되는 항목을 분석
- 한쪽에서만 처리할 수 있는 데이터는 한쪽으로 통합하여 불필요한 중복 로딩 제거
- 운영 시 양쪽 데이터를 동기화해야 하는 관리 부담 감소

#### 📈 개선 효과 요약
| 항목 | 개선 전 | 개선 후 |
|------|---------|---------|
| 데이터 관리 | 맵 데이터와 스태틱 시트 양쪽에서 중복 관리 | **한쪽에서만 처리하도록 통합** |
| 운영 공수 | 양쪽 동기화 필요, 불일치 이슈 발생 가능 | **단일 소스로 운영 공수 절감** |
